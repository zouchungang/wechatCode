...
(function () {
    ...
    //身份证号：只能填写大X和数字
    jQuery.validator.addMethod("idcardcode", function (value, element) {
        return this.optional(element) || /(^\d{18}$)|(^\d{17}(\d|X)$)/.test(value);
    }, "请填写正确身份证号");

    //只填写大写字母
    jQuery.validator.addMethod("uppercase", function (value, element, params) {
        var regex = /^[A-Z]*$/g;
    ...
    // 门票编码验证
    jQuery.validator.addMethod("Tcode", function (value, element) {
        var reg = /^[T]{1}[0-9]{10}$/;
    ...
    //  统一编码格式验证，数字或字母,6到15位
    jQuery.validator.addMethod("codeValidate", function (value, element) {
        var length = value.length;
...
//登陆名称验证 至少包含一位字母
jQuery.validator.addMethod("WordAndNum", function (value, element) {
    var reg = /[a-zA-Z]{1,}/;
    var length = value.length;
...
jQuery.validator.addMethod("integerZ", function (value, element) {
    return this.optional(element) || /^\d+$/.test(value);
}, "必须是正整数");
