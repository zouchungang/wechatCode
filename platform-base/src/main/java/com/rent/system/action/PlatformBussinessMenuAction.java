 ...
 */
@Controller
@RequestMapping("/platform/auth/bussinessMenu")
public class PlatformBussinessMenuAction extends BaseController {
    ...
    public ModelAndView index(HttpServletRequest request) {
        return new ModelAndView("platform/auth/basemenu/index");
    }
    ...
    public String remoteName(HttpServletRequest request, @RequestParam(value = "name") String name, @RequestParam(value = "id") String id, @RequestParam(value = "parentId") String parentId) {
        if (id != null && id.length() > 0) {//有id代表是修改时候的验证，验证是否与老的一样
            ...
            if (pd != null && name != null && name.trim().equals(pd.getMenuName())) {
                return "true";
            } else {
                long c = bussinessMenuService.findCountByName(name, parentId);
                if (c == 0) {
                    return "true";
                } else {
                    return "该名称不可用";
